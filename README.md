...
> 新增功能：**统一 API 网关 (3,701行)** | **IDE 兼容层 (2,484行)** | **Augment 兼容层 (1,929行)** | **思维签名缓存系统 (9,013行)** | **智能降级系统** | **多后端支持**
>
> 📖 **重构文档**: 查看 [docs/refactor/](./docs/refactor/) 了解架构设计和重构计划
...
### 🖥️ IDE 兼容层 (IDE Compatibility Layer)
...
- ✅ **思维签名处理**: 自动注入和验证 `thoughtSignature`
- ✅ **工具调用循环恢复**: 检测并修复工具调用循环问题

**技术细节**:
- 自动检测请求格式（OpenAI vs Gemini vs Anthropic）
- 透明地处理格式转换，无需客户端修改
- 支持 Gemini 3 的 `thoughtSignature` 要求
- 三层缓存架构（内存 → SQLite → 持久化）

---

### 🔄 Augment 兼容层 (Augment Compatibility Layer)
...
**特性**:
- ✅ 支持 Augment 的 `chat-stream` 端点
- ✅ 自动转换 Augment 节点格式到 OpenAI 格式
- ✅ 支持工具调用和工具结果的双向转换
- ✅ 完整的类型检查和验证

---

### 💾 思维签名缓存系统 (Signature Cache System)
...
| 层级 | 实现 | 特点 |
|------|------|------|
| **L1: 内存缓存** | `memory_cache.py` | 极速访问，自动过期 |
| **L2: SQLite 缓存** | `signature_database.py` | 持久化存储，支持查询 |
| **L3: 持久化存储** | 文件系统 | 长期存储，支持迁移 |

**核心功能**:
- ✅ **思维签名缓存**: 缓存 Gemini 3 的 `thoughtSignature`，避免重复计算
- ✅ **工具签名缓存**: 缓存工具调用的签名，提高 IDE 兼容性
